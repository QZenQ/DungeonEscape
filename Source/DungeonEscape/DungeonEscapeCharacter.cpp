...
void ADungeonEscapeCharacter::Interact()
{
	...
	FVector End = Start + (FirstPersonCameraComponent->GetForwardVector() * MaximumInteractionDistance);

	DrawDebugLine(GetWorld(), Start, End, FColor::Red, false, 5.0f);

	FCollisionShape InteractionSphere = FCollisionShape::MakeSphere(InteractionRadiusSphere);

	//DrawDebugSphere(GetWorld(), Start, InteractionRadiusSphere, 20, FColor::Blue, false, 5.0f);
	DrawDebugSphere(GetWorld(), End, InteractionRadiusSphere, 20, FColor::Green, false, 5.0f); 

	//UE_LOG(LogTemp, Warning, TEXT("Interact action triggered in %s"), *GetName());
	...
	if (HasHit)
	{
		...
		if (HitActor->ActorHasTag("Lock"))
		{
			...
			if (Lock)
			{
				if (!Lock->GetIsKeyPlaced())
				{
					...
					{
						UE_LOG(LogTemp, Warning, TEXT("Key item %s not found in inventory"), *Lock->KeyItemName);
					}
				}
				else
				{
					//lock has a key inside
					//add the key item name in the inventory
		...
		else if (HitActor->ActorHasTag("CollectableItem"))
		{
			...
			if (CollectableItem)
			{
				UE_LOG(LogTemp, Display, TEXT("Collectable Item is hit %s"), *CollectableItem->ItemName);
